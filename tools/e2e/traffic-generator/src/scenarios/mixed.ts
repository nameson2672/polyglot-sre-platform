...
// 80% valid, 20% validation fail (missing items)
async function makeRequest(pool: Pool, cfg: ScenarioConfig): Promise<RequestResult> {
  ...
  // Build body based on scenario weight
  let body: string;
  if (roll < 0.8) {
    // 80%: valid checkout
    body = JSON.stringify({
      items: [
        { sku: randomItem(SKUS), qty: Math.ceil(Math.random() * 3), unit_price_cents: 1000 },
      ],
      payment_method: 'card',
      currency: 'USD',
    });
  } else {
    // 20%: intentionally invalid (missing items → 400)
    body = JSON.stringify({ payment_method: 'card', currency: 'USD' });
  }
